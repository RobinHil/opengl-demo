...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "OpenGL - TP2", NULL, NULL);
    if (window == NULL)
    {
    ...
    std::string vertexShaderPath = std::string(_resources_directory).append("shader/simple.vert");
    ...
    std::string fragmentShaderPath = std::string(_resources_directory).append("shader/simple.frag");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // model = glm::rotate(model, (float)glfwGetTime() + glm::radians(12.0f), glm::vec3(1.0f, 0.0f, 0.0f));
        // model = glm::rotate(model, (float)glfwGetTime() * glm::radians(-37.0f), glm::vec3(0.0f, 1.0f, 0.0f));
